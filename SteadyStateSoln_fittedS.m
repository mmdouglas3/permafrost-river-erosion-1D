function [Eent, Ethaw, qw, etaSS, Eent_star, Tw_star, U_star, T0_star, Cic_star, Ch_star, Kts2_star] = ...
    SteadyStateSoln_fittedS(Cf, U, S, D, Twater, Lambda, Tbank0, Tf)
...
load('FittedSfunction.mat','Sfunc');
s = 10^(Sfunc.Coefficients.Estimate(1) + Sfunc.Coefficients.Estimate(2)*log10(abs(Eent_star)) + ...
    Sfunc.Coefficients.Estimate(3)*log10(abs(T0_star)) + ...
    Sfunc.Coefficients.Estimate(4)*log10(U_star) + ...
    Sfunc.Coefficients.Estimate(5)*log10(abs(Ch_star)) + ...
    Sfunc.Coefficients.Estimate(6)*log10(abs(Cic_star)) + ...
    Sfunc.Coefficients.Estimate(7)*log10(abs(Tw_star)) + ...
    Sfunc.Coefficients.Estimate(8)*log10(Kts2_star));
